...
try {
    ...
    $stmt1 = $pdo->prepare("SELECT * FROM project_allocation WHERE faculty_id = :faculty_id AND com_status = '1'");
    $stmt1->execute([
    ...
    if ($numrow12) {
        ...
        $stmt1 = $pdo->prepare("SELECT poc FROM project_applications WHERE faculty_id = :faculty_id AND status = '2'");
        $stmt1->execute([
            ':faculty_id'=> $faculty_id
        ]);
                    ...
                    $output .= '
                                <tr class="border-b :border-gray-700">
                                    ...
                                    <td class="px-4 py-3 ">
                                       ...
                                       <!-- Modal toggle -->
                                    <a type="button" href="../template/faculty_completed_progress_report.php?post_id=' . $row['post_id'] . '"
